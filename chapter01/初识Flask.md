...
4. Virtualenv
5. Pipfile(https://github.com/pypa/pipfile): 用来替代难于管理的requirements.txt文件
6. python-dotenv(https://github.com/theskumar/python-dotenv)
7. watchdog(https://pythonhosted.org/watchdog/): 用于监测文件变动
...
3. 安装Flask：
   在目标目录下执行`pipenv install flask`，安装Flask到当前虚拟环境中（不用激活环境）。
   Flask会同时安装5个依赖包：
   * Jinja2：模板渲染引擎；
...
2. URL路由规则`app.route(URL规则)`: 该装饰器接收URl规则（不是URL），注册路由，进行URl和函数之间的映射。
   ...
   URl规则为字符串，**必须以斜杠开始**。输入的URl为想对URL，不包括域名。
...
Flask通过Click内置了一个命令行交互界面CLI（command line interface）系统。装Flask后，会自动添加一个flask命令脚本，我们可以通过flask命令执行内置命令、扩展提供的命令或是我们自己定义的命令。
...
* `.env`：用来存储包含敏感信息的环境变量，如用户名、密码等。(**注意不要提交到公共仓库中**)
...
以上参数还可以通过环境变量来设置：`FLASK_RUN_HOST`和`FLASK_RUN_PORT`，环境变量的通用格式为`FALSK_<COMMAND>_<OPTION>`.
...
* 调试器：在网页上显示错误追踪信息，以及进行调试等，调试时需要使用启动时显示的PIN码。
* 重载器：当代码修改后，重载器监测到文件变动，然后重新启动服务器，使修改及时生效。
  ...
  **注意**：如果项目中使用了单独的CSS或JavaScript文件时，那么浏览器可能会缓存这些文件，从而导致对文件做出的修改不能立刻生效。在浏览器中，我们可以按下Crtl+F5或Shift+F5执行硬重载（hard reload），即忽略缓存并重载（刷新）页面。

## 13. Flask中的上下文指什么？

上下文（context）是为了程序正常运行，被临时保存下来的相关状态和数据。可分为**程序上下文**和**请求上下文**。

使用`flask shell`可以进入python交互模式，此时，会自动导入当前程序实例中的app示例。
...
配置变量通过Flask对象的`app.config`属性作为统一接口来获取和设置。Config类继承自字典类，可以向操作字典一样操作配置。

此外和可以将配置写入python文件、JSON文件或Python类中，然后导入配置。
...
如果程序中想要获取每个URL，可以使用`url_for()`方法进行获取。其第一个参数就是端点值。如果URL规则中含有动态参数，则也要传入相应的参数。
